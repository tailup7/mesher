...
if __name__=="__main__":
    ...
    P1, id_to_idx_1 = as_array_nodes(surfacenodes1)  # shape: (N1,3)
    P2, id_to_idx_2 = as_array_nodes(surfacenodes2)  # shape: (N2,3)
    ...
    for i, idx2 in enumerate(nn_idx):
        surfacenodes1[i].closest_surfacenode_id = int(idx2) + 1

    # 三角形頂点の座標アクセスを速くするためのヘルパ
    # tri.nodeX.id -> index -> 座標 という流れにする
    def tri_vertices_coords_in_surf2(tri):
        A = surfacenode_dict2[tri.node0.id]
        B = surfacenode_dict2[tri.node1.id]
        C = surfacenode_dict2[tri.node2.id]
        return A, B, C

    # 片側Hausdorff距離（ノード→最近傍点の属する三角形群への点-三角形距離の最小）
    haus = np.empty(len(surfacenodes1), dtype=float)

    for i, nd1 in enumerate(surfacenodes1):
        # 最近傍ノード（surface2側）のインデックス（0始まり）
        j2 = nd1.closest_surfacenode_id - 1
        nb_node2 = surfacenodes2[j2]
        ...
        for tri_id in rel_tri_ids:
            tri = surfacetriangle_dict2[tri_id]
    ...
    myio.write_vtk_hausdorff(surfacenodes1, surfacetriangles1, haus.tolist())
